...
namespace EL
{
    public class EL_VoltCalculator : MiniGameBase
    {
        ...
        void Update()
        {
            float distanceA = Vector2.Distance(transform.position, electroA.position); //プローブと電極Aの距離r
            float distanceB = Vector2.Distance(transform.position, electroB.position); //プローブと電極Bの距離r'
            ...
            {
                volt = V_0/2f * Mathf.Log(0.5f/D * distanceB/distanceA, Mathf.Exp(1)) / Mathf.Log(0.5f/D, Mathf.Exp(1));
